...
public class HebrewBigDecimalToBankingMoneyConverter implements BigDecimalToStringConverter {
	private final String DECIMAL_PREFIX, DECIMAL_SUFFIX, CURRENCY;
	...
	public HebrewBigDecimalToBankingMoneyConverter(HebrewIntegerToWordsConverter converter, HebrewValues values) {
		...
		DECIMAL_PREFIX = values.decimalPrefix();
		DECIMAL_SUFFIX = values.decimalSuffix();
	...
	public String asWords(BigDecimal value) {
		...
		return format("%s %s %s%s %s", asWord(units, GenderType.MASCULINE), CURRENCY, DECIMAL_PREFIX, asWord(decimal, GenderType.FEMININE), DECIMAL_SUFFIX);
	}

	private void validate(BigDecimal value) {
		checkArgument(value.scale() <= MAX_DECIMAL_PLACES, "can't transform more than %s decimal places for value %s", MAX_DECIMAL_PLACES,
				value);
